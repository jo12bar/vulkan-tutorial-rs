use std::{borrow::Cow, ffi::CStr, fmt, hash, ops, os::raw::c_char};

/// A fixed-length array containing a null-terminated string.
///
/// # Equality / Hashing
///
...
impl<const N: usize> StringArray<N> {
    ...
    ///
    /// Panics if `array` does not contain a null terminater.
    #[inline]
    ...
    /// Returns a reference to the underlying character array for this string array.
    #[inline]
    pub fn as_array(&self) -> &[c_char; N] {
        &self.0
    }

    /// Gets a reference to this string array as a slice of bytes.
    #[inline]
    pub fn as_bytes(&self) -> &[u8] {
        unsafe { self.as_array().align_to::<u8>().1 }
    }

    /// Gets a reference to this string array as a standard, borrowed C string.
    #[inline]
